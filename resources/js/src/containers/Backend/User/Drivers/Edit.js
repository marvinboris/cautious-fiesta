...
class Add extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.backend.drivers.driver && prevState.first_name === '') {
            const { backend: { drivers: { driver } } } = nextProps;
            return updateObject(prevState, { ...driver });
        }
        return prevState;
    }
    ...
    submitHandler = async e => {
        e.preventDefault();
        await this.props.patch(this.props.match.params.driverId, e.target);
    }
    ...
    render() {
        let {
            content: {
                cms: {
                    pages: { components: { form: { save, selected_file } }, backend: { pages: { drivers: { title, edit, index, form } } } }
                }
            },
            backend: { drivers: { loading, error, message, driver, categories } },
        ...
        if (loading) content = <Col xs={12}>
            ...
            content = (
                <>
                    <Row>
                        <Form onSubmit={this.submitHandler} icon={faUserTag} title={edit} list={index} link="/user/drivers" innerClassName="row" className="shadow-sm">
                            <Col lg={8}>
                            ...
                            <Col lg={4}>
                                <div className="embed-responsive embed-responsive-1by1 bg-soft border border-light d-flex justify-content-center align-items-center w-60 mx-auto" style={{ cursor: 'pointer', background: photo && `url("${photo}") no-repeat center`, backgroundSize: 'cover' }} onClick={this.fileUpload}>
                                    {photo && (photo !== driver.photo) && <div className="text-center text-green">
        ...
        return (
            <>
                <div className="bg-soft py-4 pl-5 pr-4 position-relative">
                    <Breadcrumb items={[{ to: '/user/drivers', content: index }]} main={edit} icon={faUserTag} />
...
const mapDispatchToProps = dispatch => ({
    get: id => dispatch(actions.getDriver(id)),
    patch: (id, data) => dispatch(actions.patchDrivers(id, data)),
